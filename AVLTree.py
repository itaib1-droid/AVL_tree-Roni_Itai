...
class AVLTree(object):
	...
	def double_rotation_left_del(self, node):
		...
		left_grandchild = new_root.left
		right_grandchild = new_root.right
		...
		right_child.left = right_grandchild
	...
	def delete(self, node):
		...
		# Special case: deleting the root - create virtual parent
		if node == self.get_root():
			prnt = AVLNode(None, None)
			right_son = True
		else:
			prnt = node.parent
			##i'm a right or left child
		...
		if node.right.is_real_node() and node.left.is_real_node():
			...
			if right_son and prnt.is_real_node():
				prnt.right = suc
			elif not right_son:
				prnt.left = suc           
		...
		# node has a left child, or is a leaf
			...
			if node == self.get_root():
				...
				self.update_max() # find new max properly
